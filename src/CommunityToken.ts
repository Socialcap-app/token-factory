...
import {
  CommunityTokenAdmin,
  CommunityTokenAdminBase,
} from "./CommunityTokenAdmin";

interface CommunityTokenDeployProps extends Exclude<DeployArgs, undefined> {
  /** The token symbol. */
  symbol: string;
...
export const CommunityTokenErrors = {
...
export class CommunityToken extends TokenContractV2 {
  @state(UInt8)
  ...
  // If you want to have a custom contract, overwrite this by setting CommunityToken.AdminContract to
  // your own implementation of CommunityTokenAdminBase.
  static AdminContract: new (...args: any) => CommunityTokenAdminBase =
    CommunityTokenAdmin;
  ...
  async deploy(props: CommunityTokenDeployProps) {
  ...
  public async getAdminContract(): Promise<CommunityTokenAdminBase> {
    const admin = await Provable.witnessAsync(PublicKey, async () => {
      let pk = await this.admin.fetch();
      assert(pk !== undefined, CommunityTokenErrors.noAdminKey);
      return pk;
    });
    this.admin.requireEquals(admin);
    return new CommunityToken.AdminContract(admin);
  }
  ...
  async setAdmin(admin: PublicKey) {
    ...
    canChangeAdmin.assertTrue(CommunityTokenErrors.noPermissionToChangeAdmin);
    this.admin.set(admin);
  ...
  async mint(recipient: PublicKey, amount: UInt64): Promise<AccountUpdate> {
    this.paused
      .getAndRequireEquals()
      .assertFalse(CommunityTokenErrors.tokenPaused);
    const accountUpdate = this.internal.mint({ address: recipient, amount });
    const adminContract = await this.getAdminContract();
    const canMint = await adminContract.canMint(accountUpdate);
    canMint.assertTrue(CommunityTokenErrors.noPermissionToMint);
    recipient
      .equals(this.address)
      .assertFalse(CommunityTokenErrors.noTransferFromCirculation);
  ...
  async pause() {
    ...
    canPause.assertTrue(CommunityTokenErrors.noPermissionToPause);
    this.paused.set(Bool(true));
  ...
  async resume() {
    ...
    canResume.assertTrue(CommunityTokenErrors.noPermissionToResume);
    this.paused.set(Bool(false));
  ...
  async transfer(from: PublicKey, to: PublicKey, amount: UInt64) {
    ...
    to.equals(this.address).assertFalse(
      CommunityTokenErrors.noTransferFromCirculation
    );
  ...
  private checkPermissionsUpdate(update: AccountUpdate) {
    ...
    assert(
      ...
      CommunityTokenErrors.noPermissionChangeAllowed
    );
  }
  ...
  async approveBase(updates: AccountUpdateForest): Promise<void> {
    ...
    this.forEachUpdate(updates, (update, usesToken) => {
      ...
      totalBalance.isPositiveV2().assertFalse(CommunityTokenErrors.flashMinting);
    });
    totalBalance.assertEquals(
      Int64.zero,
      CommunityTokenErrors.unbalancedTransaction
    );
  }
